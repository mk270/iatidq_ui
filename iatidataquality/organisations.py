    ...
    def org_as_dict(o):
        return {
            "org_url": url_for('organisations', 
                               organisation_code=o.organisation_code),
            "organisation_name": o.organisation_name,
            "coverage": {
                "found": getattr(coverage_data[o.id], "found", ""),
                "total": getattr(coverage_data[o.id], "total", ""),
                "pct": getattr(coverage_data[o.id], "pct", "")
                }
            }

    payload = {
        "admin": usermanagement.check_perms('admin'),
        "org_new_url": url_for('organisation_new'),
        "organisation": [ org_as_dict(o) for o in organisations ]
        }
...
def organisations(organisation_code=None):
    ...
    def org_as_dict(o):
        org = dict(o.as_dict())
        org["org_url"] = url_for('organisations', 
			   organisation_code=o.organisation_code)
        return org
    ...
    return render_template("organisations.html", json_data=json_data,
                           **template_args)
...
def all_organisations_publication_index_csv_history():
    organisations = ui_support.all_organisations_unsorted()
